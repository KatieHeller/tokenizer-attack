# Gets merge frequencies for all programming languages from Red Pajama dataset

model_name=qwen3
experiment_dir=experiments/llm_tokenizers/$model_name
programming_languages_corpus_dir=/scratch/network/kh3329/tokenizer-attack/github/scrubbed

for subdir in "$programming_languages_corpus_dir"/*/; do
    ...
    if mkdir "$lock_dir" 2>/dev/null; then
        ...
        if [ -d "$output_dir" ]; then
            ...
            python -m dump_frequencies \
                ...
                --lang_code "$lang_code" \
                --corpus_dir "$programming_languages_corpus_dir" \
